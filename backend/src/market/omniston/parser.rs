...
use crate::market::types::{OmnistonEvent, Quote};
...
pub fn parse_omniston_event(
    raw: &str,
    _side: &crate::market::types::QuoteSide,
    ...
    if let Some(ack) = event.get("ack")
        && let Some(rfq_id) = ack.get("rfq_id").and_then(|v| v.as_str())
    {
    ...
    if let Some(q) = event.get("quote_updated") {
        let quote: Quote = serde_json::from_value(q.clone())?;
...
mod tests {
    use super::*;
    use crate::market::types::QuoteSide;
    use serde_json::json;
